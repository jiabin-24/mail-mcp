...
class EmailSendQueueStoreBase:
    ...
    def __init__(self, token_provider: Callable[[], str | None], *, table_name: str | None = None) -> None:
        ...
        self._table_name = (table_name or os.getenv("AZURE_STORAGE_TABLE_NAME") or "EmailSendQueue").strip()
        ...
        self._credential = table_context.credential
        self._table_client = table_context.table_client

    def enqueue_send_job(self, req: MailboxCreateSendJobInput) -> dict[str, Any]:
        user_upn = self._resolve_user_upn()
        if self._find_scheduled_job_for_draft(user_upn=user_upn, draft_email_id=req.draft_email_id) is not None:
            return {
                "status": "no_change",
        ...
        entity: dict[str, Any] = {
            ...
            "schedulesendtime": self._to_queue_time(req.schedule_send_time),
            "status": req.status,
    ...
    def dispatch_pending_jobs(self) -> dict[str, Any]:
        ...
        for entity in entities:
            ...
            try:
                self._send_draft_for_job(user_upn=user_upn, draft_email_id=draft_id)
                ...
                failed_count += 1
                failed.append({"job_id": row_key, "draftemailid": draft_id, "error": str(exc)})
                self._update_job_status(
                    partition_key=partition_key,
                    row_key=row_key,
                    status="failed",
    ...
    def _find_scheduled_job_for_draft(self, *, user_upn: str, draft_email_id: str) -> dict[str, Any] | None:
        escaped_upn = user_upn.replace("'", "''")
    ...
    def _update_job_status(self, *, partition_key: str, row_key: str, status: str, sent_time: str, error: str = "") -> None:
        entity = {
    ...
    def _resolve_user_upn(self) -> str:
        raise NotImplementedError("Backend adapter must resolve current user UPN")

    def _send_draft_for_job(self, *, user_upn: str, draft_email_id: str) -> None:
        raise NotImplementedError("Backend adapter must send an eligible queued draft")

    def _to_queue_time(self, value: Any) -> str:
        if value is None:
            return ""
...
__all__ = ["EmailSendQueueStoreBase"]
